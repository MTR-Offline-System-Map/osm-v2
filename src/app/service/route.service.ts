...
export class RouteVariationService extends SelectableDataServiceBase<void, Route> {
	...
	constructor() {
		...
		super(routeId => {
			const routeStationDetails: { id: string, name: string, platform: string, duration: number, durationSeconds: number, dwellTime: number, dwellTimeSeconds: number }[] = [];
			...
			if (selectedRoute) {
				for (let i = 0; i < selectedRoute.routePlatforms.length; i++) {
					...
					routeStationDetails.push({id: station.id, name: station.name, platform: name, duration, durationSeconds: Math.round(duration / 1000), dwellTime, dwellTimeSeconds: Math.round(dwellTime / 1000)});
					routeVehicles.push([]);
