...
export class StationService extends SelectableDataServiceBase<{ currentTime: number, data: { arrivals: DataResponse[] } }, Station> {
	...
	public readonly routesAtStation: { name: string, platforms: string[], variations: string[], number: string, color: number, typeIcon: string, hidden: boolean }[] = [];
	...
	public setStation(stationId: string, zoomToStation: boolean) {
		...
		this.dataService.routes.forEach(({name, number, color, type, routePlatforms, hidden}) => {
			const routePlatform = routePlatforms.find(routePlatform => routePlatform.station.id === this.getSelectedData()?.id);
			if (routePlatform) {
