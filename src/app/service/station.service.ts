...
export class StationService extends SelectableDataServiceBase<{ currentTime: number, data: { arrivals: DataResponse[] } }, Station> {
	...
	public readonly routesAtStation = signal<{ name: string, platforms: string[], variations: string[], number: string, color: number, typeIcon: string, hidden: boolean }[]>([]);
	...
	public setStation(stationId: string, zoomToStation: boolean) {
		...
		const newRoutes: Record<string, { name: string, platforms: string[], variations: string[], number: string, color: number, typeIcon: string, hidden: boolean }> = {};
		this.dataService.routes().forEach(({name, number, color, type, routePlatforms, hidden}) => {
			const routePlatform = routePlatforms.find(routePlatform => routePlatform.station.id === this.selectedData()?.id);
			if (routePlatform) {
				...
				if (key in newRoutes) {
					...
					pushIfNotExists(newRoutes[key].platforms, routePlatform.name);
				} else {
					newRoutes[key] = {name: name.split("||")[0], platforms: [routePlatform.name], variations: [variation], number, color, typeIcon: ROUTE_TYPES[type].icon, hidden};
				}
			}
		});
